...
export const PaymentModal: React.FC<PaymentModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-end sm:items-center justify-center p-0 sm:p-4 z-50">
      {/* Modal Container - Structure flex pour séparer header, content, footer */}
      <div className="bg-white rounded-t-2xl sm:rounded-xl shadow-2xl w-full max-h-[calc(100vh-80px)] sm:max-h-[90vh] sm:max-w-lg flex flex-col">
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden">
          ...
          <div className="flex-shrink-0 p-4 sm:p-5 border-t border-slate-200 bg-white">
            <div className="flex flex-col sm:flex-row gap-2 sm:gap-3">
              <button
                type="button"
                onClick={onClose}
                disabled={submitting}
                className="flex-1 px-4 py-3 sm:py-2.5 border border-slate-300 text-slate-700 rounded-lg text-sm sm:text-base hover:bg-slate-50 disabled:opacity-50 font-medium"
              >
                Annuler
              </button>
              <button
                ...
                className="flex-1 px-4 py-3 sm:py-2.5 bg-green-500 text-white rounded-lg text-sm sm:text-base hover:bg-green-600 disabled:opacity-50 font-semibold"
              >
