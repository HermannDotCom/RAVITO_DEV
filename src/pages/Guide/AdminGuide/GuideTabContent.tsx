...
interface Props {
  tab: GuideTab;
  accentColor?: 'orange' | 'slate';
}

export const GuideTabContent: React.FC<Props> = ({ tab, accentColor = 'slate' }) => {
  const borderColor = accentColor === 'orange'
    ? 'border-orange-400'
    : 'border-slate-500';
  const bgColor = accentColor === 'orange'
    ? 'bg-orange-50 dark:bg-orange-900/10'
    : 'bg-slate-50 dark:bg-slate-800/40';
  const badgeBg = accentColor === 'orange'
    ? 'bg-orange-100 dark:bg-orange-900/30 text-orange-600 dark:text-orange-400'
    : 'bg-slate-100 dark:bg-slate-700 text-slate-600 dark:text-slate-300';

  return (
    <div className="space-y-8">
      <p className={`text-gray-600 dark:text-gray-300 text-base leading-relaxed border-l-4 ${borderColor} pl-4 ${bgColor} py-3 pr-4 rounded-r-lg`}>
        {tab.intro}
      </p>
        ...
        {tab.steps.map((step) => (
          ...
          >
            <div className="flex-shrink-0">
              <div className={`w-9 h-9 rounded-full flex items-center justify-center font-bold text-sm ${badgeBg}`}>
                {step.number}
              </div>
            </div>
