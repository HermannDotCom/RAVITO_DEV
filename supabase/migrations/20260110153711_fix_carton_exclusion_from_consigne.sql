/*
  # Fix CARTON Exclusion from Consigne

  ## Problem
  CARTON packages (CARTON24, CARTON12, CARTON6, etc.) are disposable and should 
  NEVER be counted as consignable items. The current accept_supplier_offer function
  incorrectly includes them in packaging_snapshot.

  ## Solution
  Update accept_supplier_offer function to exclude CARTON types when calculating
  packaging_snapshot. A package is consignable IF AND ONLY IF:
  1. withConsigne = true on the order item
  2. AND product.consign_price > 0
  3. AND product.crate_type DOES NOT START WITH 'CARTON'

  ## Changes
  - Add consign_price > 0 check
  - Add exclusion for crate_type starting with 'CARTON'
*/

-- Update accept_supplier_offer function to exclude CARTON types from packaging_snapshot
  ...
  BEGIN
    ...
    FROM (
      ...
      WHERE COALESCE((item->>'withConsigne')::BOOLEAN, false) = true
        AND p.consign_price > 0  -- Nouveau: prix consigne > 0
        AND p.crate_type NOT LIKE 'CARTON%'  -- Nouveau: exclure cartons
