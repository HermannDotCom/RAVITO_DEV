/*
  # Correction insertion order_items avec unit_price

  ## Problème
  La fonction accept_supplier_offer insère order_items sans unit_price,
  causant l'erreur "unit_price violates not-null constraint"

  ## Solution
  Modifier la fonction pour récupérer unit_price depuis products lors de l'insertion
*/
...
BEGIN
  ...
  -- 11. Valider et insérer les nouveaux order_items depuis l'offre AVEC unit_price
  ...
  -- Insérer les items avec unit_price récupéré depuis products
  BEGIN
    INSERT INTO order_items (order_id, product_id, quantity, unit_price, with_consigne)
    SELECT
      p_order_id,
      (item->>'productId')::UUID,
