/*
  # Ajouter packaging_snapshot à orders et à la vue orders_with_coords
  
  1. Ajoute la colonne packaging_snapshot à la table orders
  2. Met à jour la fonction accept_supplier_offer pour calculer le snapshot (SANS les cartons)
  3. Recrée la vue orders_with_coords pour inclure packaging_snapshot
  
  ## Format packaging_snapshot
  ```json
  {
    "C12": 2,
    "C24": 1,
  ...
  Exclut les types CARTON (emballages jetables non consignés).
*/

-- 1. Ajouter packaging_snapshot column à orders
ALTER TABLE orders ADD COLUMN IF NOT EXISTS packaging_snapshot JSONB;

-- 2. Mettre à jour la fonction accept_supplier_offer avec calcul du snapshot (EXCLUANT CARTON)
...
BEGIN
  ...
  -- EXCLUANT les CARTON (emballages jetables)
  BEGIN
  ...
  -- 10. Mettre à jour la commande AVEC packaging_snapshot
  UPDATE orders
  SET
    status = 'awaiting-payment',
    supplier_id = v_offer.supplier_id,
  ...
  -- 12. Insérer les nouveaux order_items
...
-- 3. Recréer la vue orders_with_coords pour inclure packaging_snapshot
...
-- Commentaires
COMMENT ON COLUMN orders.packaging_snapshot IS 'Snapshot of packaging quantities grouped by crate_type, calculated at offer acceptance (EXCLUDES CARTON types)';
COMMENT ON FUNCTION accept_supplier_offer IS 'Accepte une offre fournisseur avec calcul du snapshot des emballages groupés par type de casier (EXCLUANT les CARTON jetables)';
